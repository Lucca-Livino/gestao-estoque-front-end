...
export default function PerfilPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (!formData.telefone || !telefoneRegex.test(formData.telefone)) {
        toast.error("Telefone deve estar no formato (XX) XXXX-XXXX ou (XX) XXXXX-XXXX");
        return;
      }
  ...
  const handlePhotoChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!TiposPermitidos.includes(file.type)) {
      toast.error("Formato de arquivo não suportado. Use apenas: JPEG, PNG, GIF ou WEBP");
      return;
    }
          ...
          <div className="bg-[#0344DA]/90 backdrop-blur-sm rounded-xl sm:rounded-3xl p-4 sm:p-8 md:p-12 shadow-xl">
            <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 lg:gap-12">
              <div className="w-full lg:w-1/3 flex flex-col items-center justify-center">
                <div className="flex flex-col items-center space-y-4 sm:space-y-6 w-full">
                  <div className="relative flex justify-center w-full">
                    <Avatar data-test="avatar-usuario" className="h-32 w-32 sm:h-40 sm:w-40 lg:h-48 lg:w-48 border-4 border-white/20 shadow-2xl">
                      <AvatarImage 
                  ...
                  <h2 data-test="nome-usuario-exibido" className="text-xl sm:text-2xl lg:text-3xl font-bold text-white text-center">
                    {nomeUsuario}
                  </h2>

                  <p data-test="perfil-usuario" className="text-base sm:text-lg text-white/90 font-medium">
              ...
              <div className="flex-1">
                <h1 data-test="titulo-informacoes-conta" className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white text-center mb-6 sm:mb-8 lg:mb-12">
