...
export function FuncionarioListagem({
  ...
  // Adaptação da função de impressão para o modelo Funcionario
  const handlePrintFuncionario = () => {
    ...
    // Adaptando o status 'ativo' para o badge (true = Ativo, false = Inativo)
    const statusBadge = createStatusBadge(funcionario.ativo);
    ...
    const sections = [
      {
        ...
        fields: [
          ...
          { label: "Nome do Funcionário", value: funcionario.nome_usuario || "-" },
  ...
  const desativarMutation = useMutation({
    mutationFn: async (id: string) => {
      // Endpoint e payload alterados para Funcionário
      const result = await fetchData(
        `/funcionarios/${id}`,
        "PATCH",
        undefined,
        {
          ativo: false, // Alterado de status para ativo
        }
      );
      return result;
    },
          ...
          <DialogTitle>
            {funcionario ? (
              <div className="bold text-1xl flex gap-2">
                ...
                />
                {funcionario.ativo && ( // Botão de desativar só aparece se estiver ativo
                  <AlertDialog>
                ...
                <FieldGroup>
                  <div className="flex flex-row gap-1">
                    ...
                    <Field>
                      ...
                      <Input
                        id="status"
                        value={funcionario.ativo ? "Ativo" : "Inativo"}
                        readOnly={true}
                      />
                    </Field>
                  </div>
