    ...
    async function loadData() {
      ...
      try {
        ...
        if (
          ...
          produtosResponse.data.produtos.length === 0
        ) {
        ...
        const produtosFormatadosData: Produto[] =
          produtosResponse.data.produtos.map((p: any) => ({
            _id: p._id,
        ...
        setTotalProdutos(
          produtosResponse.data.total || produtosFormatadosData.length
        );

        const codigosProdutos = produtosResponse.data.produtos.map(
          (p: any) => p.codigo_produto
        );
        ...
        if (!movimentacoesResponse.error && movimentacoesResponse.data) {
          console.log(
            `Total de movimentações: ${movimentacoesResponse.data.docs.length}`
          );
          console.log(
            `Códigos de produtos da categoria ${categoria} (${codigosProdutos.length}):`,
            codigosProdutos.slice(0, 5)
          );

          const movimentacoesCategoria = movimentacoesResponse.data.docs.filter(
            (mov) =>
              mov.produtos.some((p) =>
                codigosProdutos.includes(p.codigo_produto)
              )
          );
          ...
          const dataMap = new Map<
            string,
            { entradas: number; saidas: number; mesIndex: number }
          >();
          ...
          console.log(
            `Total de movimentações da categoria ${categoria}: ${movimentacoesCategoria.length}`
          );
          ...
          movimentacoesCategoria.forEach((mov, idx) => {
            ...
            if (idx < 5) {
              console.log(
                `Movimentação ${idx + 1}: tipo=${mov.tipo}, ano=${ano}, data=${
          ...
          console.log(
            `Categoria ${categoria}: ${entradasCount} entradas, ${saidasCount} saídas processadas`
          );
          console.log(
            `Total entradas: ${somaEntradas}, Total saídas: ${somaSaidas}`
          );
