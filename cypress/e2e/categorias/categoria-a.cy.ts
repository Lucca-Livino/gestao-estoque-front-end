describe("Categoria A", () => {
  beforeEach(() => {
    ...
    cy.visit("https://garagehub.app.fslab.dev/categoria_A");
  });

  it("Deve exibir o título da categoria corretamente", () => {
    cy.getByData("titulo-categoria").should("be.visible");
    cy.getByData("titulo-categoria").should("contain", "Categoria A - Produtos de Alto Valor");
  });
  ...
  it("Deve exibir o gráfico de movimentações", () => {
    ...
    cy.getByData("titulo-chart").should("contain", "Movimentações de Produtos - Categoria A");
